  ...
  init() {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.config) {
        ...
        if (!config) {
          // If still no config, wait a bit and try again (in case injection is delayed)
          console.log('No config found, waiting 500ms and trying again...');
          await new Promise(resolve => setTimeout(resolve, 500));
          
          if (typeof window !== 'undefined' && window.config) {
            console.log('Config found after delay (injected by WebXRide)');
            config = window.config;
          } else {
