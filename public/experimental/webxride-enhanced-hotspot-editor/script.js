...
class HotspotEditor {
  constructor() {
    ...
    // Check URL for WebXRide mode and set flags accordingly
    const urlParams = new URLSearchParams(window.location.search);
    const isWebXRideUrl = urlParams.has('webxride') || window.location.href.includes('webxride=true');
    
    this.isWebXRideMode = isWebXRideUrl;
    this.webxrideContext = isWebXRideUrl ? { mode: 'webxride', timestamp: new Date().toISOString() } : null;
    this.webxrideAssets = null;

    console.log('HotspotEditor constructor - URL check:', window.location.href);
    console.log('HotspotEditor constructor - isWebXRideUrl:', isWebXRideUrl);
    console.log('HotspotEditor constructor - isWebXRideMode:', this.isWebXRideMode);
    console.log('HotspotEditor constructor - webxrideContext:', this.webxrideContext);

    this.init();
  }

  /**
   * Check if we're running in an iframe (WebXRide context)
   */
  detectIframeContext() {
    const inIframe = window !== window.top;
    const hasWebXRideParam = window.location.href.includes('webxride=true') || 
                             window.location.search.includes('webxride=true');
    
    console.log('Iframe detection - inIframe:', inIframe);
    console.log('Iframe detection - hasWebXRideParam:', hasWebXRideParam);
    
    if (inIframe || hasWebXRideParam) {
      console.log('Iframe context detected, setting WebXRide mode');
      this.forceWebXRideMode();
      return true;
    }
  ...
  /**
   * Start periodic WebXRide mode maintenance
   */
  startWebXRideModeMaintenance() {
    console.log('Starting WebXRide mode maintenance...');
    
    // Check mode every 2 seconds
    setInterval(() => {
      if (this.isWebXRideMode) {
        // Log current status
        this.checkWebXRideModeStatus();
        
        // Ensure mode is still active
        if (!this.webxrideContext) {
          console.log('WebXRide context lost, recreating...');
          this.forceWebXRideMode();
        }
      }
    }, 2000);
    
    console.log('WebXRide mode maintenance started');
  }
  ...
  init() {
    ...
    // Check for iframe context and force WebXRide mode if needed
    this.detectIframeContext();
    
    // Force WebXRide mode if URL indicates it
    if (this.isWebXRideMode) {
      this.forceWebXRideMode();
      this.startWebXRideModeMaintenance();
    }
  }

  /**
   * Check and log WebXRide mode status
   */
  checkWebXRideModeStatus() {
    console.log('=== WebXRide Mode Status Check ===');
    console.log('this.isWebXRideMode:', this.isWebXRideMode);
    console.log('this.webxrideContext:', this.webxrideContext);
    console.log('window.location.href:', window.location.href);
    console.log('window.location.search:', window.location.search);
    console.log('URL contains webxride=true:', window.location.href.includes('webxride=true'));
    console.log('================================');
  }

  /**
   * Force WebXRide mode when URL indicates it
   */
  forceWebXRideMode() {
    console.log('Forcing WebXRide mode...');
    this.isWebXRideMode = true;
    if (!this.webxrideContext) {
      this.webxrideContext = { 
        mode: 'webxride', 
        timestamp: new Date().toISOString(),
        forced: true 
      };
    }
    console.log('WebXRide mode forced - isWebXRideMode:', this.isWebXRideMode);
    console.log('WebXRide mode forced - webxrideContext:', this.webxrideContext);
    
    // Schedule periodic status checks
    if (!this.webxrideStatusInterval) {
      this.webxrideStatusInterval = setInterval(() => {
        this.checkWebXRideModeStatus();
  ...
  async saveTemplate() {
    try {
      ...
      if (exportType === 'webxride') {
        console.log('Saving to WebXRide...');
        await this.saveToWebXRide();
        return;
      }
      
      // Get template name for other export types
      const templateName = prompt("Enter template name:", "360° Interactive Tour");
      if (!templateName) return;
      
      if (exportType === "json") {
        await this.saveAsJSON(templateName);
      } else if (exportType === "project") {
        await this.saveAsCompleteProject(templateName);
      }
    } catch (error) {
  ...
  /**
   * Ensure WebXRide mode is active before showing export dialog
   */
  ensureWebXRideMode() {
    console.log('Ensuring WebXRide mode is active...');
    
    // Check URL first
    const urlHasWebXRide = window.location.href.includes('webxride=true') || 
                           window.location.search.includes('webxride=true');
    
    if (urlHasWebXRide) {
      console.log('URL indicates WebXRide mode, forcing it...');
      this.forceWebXRideMode();
      return true;
    }
    ...
    if (this.webxrideContext) {
      console.log('WebXRide context already exists, mode is active');
      return true;
    }
    
    // Check if mode flag is set
    if (this.isWebXRideMode) {
      console.log('WebXRide mode flag is already set');
      return true;
    }
    
    console.log('WebXRide mode not detected');
    return false;
  }

  /**
   * Force WebXRide button to appear in export dialog
   */
  forceWebXRideButtonInDialog(dialog) {
    console.log('Forcing WebXRide button to appear in dialog...');
    
    // Check if we're in WebXRide mode
    const isWebXRideMode = this.isWebXRideMode || 
                          this.webxrideContext || 
                          window.location.href.includes('webxride=true') ||
                          window.location.search.includes('webxride=true');
    
    if (isWebXRideMode) {
      console.log('WebXRide mode detected, ensuring button is visible...');
      
      // Find the dialog content
      const dialogContent = dialog.querySelector('div');
      if (dialogContent) {
        // Check if WebXRide button already exists
        const existingButton = dialog.querySelector('#export-webxride');
        if (!existingButton) {
          console.log('WebXRide button not found, adding it...');
          
          // Insert the WebXRide button at the top
          const webxrideSection = document.createElement('div');
          webxrideSection.style.margin = '20px 0';
          webxrideSection.innerHTML = `
            ...
            ">🚀 Save to WebXRide</button>
            <div style="font-size: 12px; color: #ccc; margin-left: 5px;">
              Save directly to WebXRide editor
            </div>
          `;
          
          // Insert before the first existing button
          const firstButton = dialogContent.querySelector('button');
          if (firstButton) {
            dialogContent.insertBefore(webxrideSection, firstButton);
          } else {
            dialogContent.appendChild(webxrideSection);
          }
          
          console.log('WebXRide button added to dialog');
          ...
          if (webxrideBtn) {
            webxrideBtn.onclick = () => {
              ...
              // Resolve with webxride type
              return Promise.resolve("webxride");
            };
          }
        } else {
          console.log('WebXRide button already exists in dialog');
        }
      }
    } else {
      console.log('WebXRide mode not detected, button will not be shown');
    }
  }

  showExportDialog() {
    return new Promise((resolve) => {
      // Ensure WebXRide mode is active before showing dialog
      this.ensureWebXRideMode();
      ...
      // Check if we're in WebXRide mode - multiple ways to check
      const isWebXRideMode = this.isWebXRideMode || 
                            this.webxrideContext || 
                            window.location.href.includes('webxride=true') ||
                            window.location.search.includes('webxride=true');
      ...
      // Debug logging
      console.log('showExportDialog - this.isWebXRideMode:', this.isWebXRideMode);
      console.log('showExportDialog - this.webxrideContext:', this.webxrideContext);
      console.log('showExportDialog - window.location.href:', window.location.href);
      console.log('showExportDialog - window.location.search:', window.location.search);
      console.log('showExportDialog - isWebXRideMode (computed):', isWebXRideMode);
      console.log('showExportDialog - this object keys:', Object.keys(this));
      console.log('showExportDialog - this object:', this);
      ...
      dialog.innerHTML = `
        <div style="background: #2a2a2a; padding: 30px; border-radius: 10px; color: white; max-width: 600px;">
          ...
          <div style="margin: 20px 0; padding: 20px; background: rgba(255, 107, 53, 0.1); border-radius: 8px; border: 1px solid rgba(255, 107, 53, 0.3);">
            <h4 style="margin-top: 0; color: #FF6B35;">🚀 Save to WebXRide</h4>
            ...
            <div style="margin: 15px 0;">
              ...
              <input type="text" id="webxride-name" value="360° Interactive Tour" 
            ...
            <div style="margin: 15px 0;">
              ...
              <textarea id="webxride-description" 
                        style="width: 100%; padding: 8px; border: 1px solid #555; border-radius: 4px; background: #333; color: white; font-size: 14px; height: 60px; resize: vertical;">Interactive 360° tour with hotspots</textarea>
            </div>
      ...
      // Force WebXRide button to appear if needed
      this.forceWebXRideButtonInDialog(dialog);
      ...
      if (isWebXRideMode) {
        ...
        if (webxrideBtn) {
          webxrideBtn.onclick = () => {
            // Collect template details from input fields
            const templateName = document.getElementById("webxride-name").value.trim() || "360° Interactive Tour";
            const templateDescription = document.getElementById("webxride-description").value.trim() || "Interactive 360° tour with hotspots";
            
            // Store the details for use in saveToWebXRide
            this.pendingWebXRideExport = { templateName, templateDescription };
            
            document.body.removeChild(dialog);
            resolve("webxride");
          };
        } else {
          console.error('WebXRide export button not found in DOM!');
        }
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * Handle direct WebXRide save (when button is clicked from main UI)
   */
  async handleDirectWebXRideSave() {
    console.log('Direct WebXRide save requested...');
    
    // Ensure WebXRide mode is active
    if (!this.ensureWebXRideMode()) {
      console.error('WebXRide mode not available for direct save');
      alert('WebXRide mode not available. Please use the export dialog instead.');
      return;
    }
  ...
  /**
   * Save template to WebXRide
   */
  async saveToWebXRide() {
    try {
      console.log('Starting WebXRide save process...');
      
      // Use collected template details or fallback to defaults
      let templateName = '360° Interactive Tour';
      let templateDescription = 'Interactive 360° tour with hotspots';
      
      if (this.pendingWebXRideExport) {
        templateName = this.pendingWebXRideExport.templateName;
        templateDescription = this.pendingWebXRideExport.templateDescription;
        // Clear the pending export data
        delete this.pendingWebXRideExport;
      }
      
      // Validate template details
      if (!templateName || templateName.trim() === '') {
        templateName = '360° Interactive Tour';
      }
      if (!templateDescription || templateDescription.trim() === '') {
        templateDescription = 'Interactive 360° tour with hotspots';
      }
      ...
      const templateData = {
        ...
        html: this.generateCompleteHTML(templateName),
        js: this.generateCompleteJS(),
        ...
        metadata: {
          ...
          sceneCount: Object.keys(this.scenes).length,
          hasAudio: this.hotspots.some(h => h.audio),
          hasNavigation: this.hotspots.some(h => h.navigationTarget),
          startingPoint: this.scenes[this.currentScene]?.startingPoint
        }
      };

      console.log('Template data generated, sending to WebXRide...');
      ...
      if (window.parent) {
        ...
        console.log('webxride-template-save message sent to parent');
        
        // Show success message and close the editor
        const successMessage = `Template "${templateName}" successfully sent to WebXRide!\n\nDetails:\n• ${templateDescription}\n• ${Object.keys(this.scenes).length} scenes\n• ${this.hotspots.length} hotspots\n• Type: 360° Interactive Tour\n\nThis window will close automatically and your project will be loaded in WebXRide.`;
        alert(successMessage);
        
        // Close the 360° editor window after a short delay to ensure message is sent
        setTimeout(() => {
          if (window.parent && window.parent !== window) {
            // If we're in an iframe, try to close the parent window or navigate away
            try {
              window.close();
            } catch (e) {
              // If we can't close, try to navigate away
              window.location.href = 'about:blank';
            }
          }
        }, 1000);
      } else {
        console.error('No parent window found for WebXRide communication');
        alert('Error: No WebXRide connection found');
      }

    } catch (error) {
      console.error('Error saving to WebXRide:', error);
      alert('Error saving template to WebXRide: ' + error.message);
    }
  }

  /**
   * Add WebXRide-specific UI elements
   */
  addWebXRideUI() {
    console.log('Adding WebXRide UI elements...');
    
    // Add save to WebXRide button
    const saveButtonContainer = document.querySelector('.button-container');
    if (saveButtonContainer) {
      const saveToWebXRideBtn = document.createElement('button');
      saveToWebXRideBtn.textContent = 'Save to WebXRide';
      saveToWebXRideBtn.className = 'btn btn-primary';
      saveToWebXRideBtn.style.marginLeft = '10px';
      saveToWebXRideBtn.addEventListener('click', () => this.handleDirectWebXRideSave());
      
      saveButtonContainer.appendChild(saveToWebXRideBtn);
      console.log('Save to WebXRide button added to main UI');
    } else {
...
AFRAME.registerComponent("editor-spot", {
  ...
  // ==================== WEBXRIDE INTEGRATION METHODS ====================

  /**
   * Initialize WebXRide integration
   */
  initWebXRideIntegration() {
    console.log('initWebXRideIntegration called');
    this.isWebXRideMode = true;
    
    // Listen for messages from WebXRide
    window.addEventListener('message', (event) => {
      console.log('Editor received message:', event.data);
      this.handleWebXRideMessage(event);
    });

    // Add WebXRide-specific UI elements
    console.log('Adding WebXRide UI...');
    this.addWebXRideUI();
    
    console.log('WebXRide integration initialized successfully');
    ...
    if (window.parent) {
      ...
      window.parent.postMessage({
        type: 'EDITOR_READY',
        data: { status: 'ready', mode: 'webxride' }
      }, '*');
    }
  },

  /**
   * Handle messages from WebXRide
   */
  handleWebXRideMessage(event) {
    console.log('Editor handling WebXRide message:', event.data);
    ...
    if (event.data && typeof event.data === 'object') {
      ...
      switch (type) {
        case 'WEBXRIDE_CONTEXT':
          console.log('Received WebXRide context:', data);
          this.webxrideContext = data;
          this.onWebXRideContextReceived(data);
          break;
        ...
        case 'CHECK_STATUS':
          ...
          window.parent.postMessage({
            ...
            data: {
              status: 'ready',
              features: ['hotspots', 'scenes', 'webxride'],
              timestamp: new Date().toISOString()
            }
          }, '*');
          break;
        ...
        case 'CLOSE_EDITOR':
          console.log('Received close request from WebXRide');
          // Handle editor close
          break;
          
        default:
          console.log('Unknown WebXRide message type:', type);
      }
    }
  },

  /**
   * Handle WebXRide context received
   */
  onWebXRideContextReceived(context) {
    console.log('WebXRide context received:', context);
    console.log('onWebXRideContextReceived called on HotspotEditor instance:', this);
    console.log('Before setting - isWebXRideMode:', this.isWebXRideMode);
    console.log('Before setting - webxrideContext:', this.webxrideContext);
    
    // Set WebXRide mode and store context
    this.isWebXRideMode = true;
    this.webxrideContext = context;
    
    console.log('After setting - isWebXRideMode:', this.isWebXRideMode);
    console.log('After setting - webxrideContext:', this.webxrideContext);
    
    if (context.assets) {
      this.webxrideAssets = context.assets;
      this.loadWebXRideAssets();
    }
    
    // Add WebXRide UI elements
    this.addWebXRideUI();
    
    console.log('WebXRide integration fully initialized');
    
    // Force a re-check of WebXRide mode
    this.forceWebXRideMode();
  },

  /**
   * Load WebXRide assets
   */
  loadWebXRideAssets() {
    if (this.webxrideAssets) {
      if (this.webxrideAssets.images) {
        this.updateSceneImageOptions(this.webxrideAssets.images);
      }
      if (this.webxrideAssets.audio) {
        this.updateAudioFileOptions(this.webxrideAssets.audio);
      }
    }
  },

  /**
   * Update scene image options with WebXRide assets
   */
  updateSceneImageOptions(images) {
    // Implementation for updating scene image options
    console.log('Updating scene image options with:', images);
  },

  /**
   * Update audio file options with WebXRide assets
   */
  updateAudioFileOptions(audioFiles) {
    // Implementation for updating audio file options
    console.log('Updating audio file options with:', audioFiles);
  },
  ...
  async saveToWebXRide() {
    try {
      ...
      const templateDescription = prompt('Enter template description:', 'Interactive 360° tour with hotspots');
      ...
      console.error('Error saving to WebXRide:', error);
      alert('Error saving template to WebXRide');
    }
  },

  /**
   * Browse WebXRide assets
   */
  browseWebXRideAssets() {
    // Request assets from WebXRide
    if (window.parent) {
      window.parent.postMessage({
        type: 'LOAD_ASSETS',
        data: ['images', 'audio']
  ...
  /**
   * Update asset lists with data from WebXRide
   */
  updateAssetLists(assets) {
    if (assets.images) {
      this.updateSceneImageOptions(assets.images);
    }
    if (assets.audio) {
      this.updateAudioFileOptions(assets.audio);
    }
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const waitForAFrame = () => {
    if (window.AFRAME && window.AFRAME.utils && window.AFRAME.utils.device) {
      ...
      // Always set up message listener for WebXRide integration
      // This ensures we can receive context even if it's sent before initWebXRideIntegration is called
      window.addEventListener('message', (event) => {
        console.log('Editor received message:', event.data);
        console.log('Editor message source:', event.source);
        console.log('Editor message origin:', event.origin);
        if (window.hotspotEditor && window.hotspotEditor.handleWebXRideMessage) {
          console.log('Calling handleWebXRideMessage on hotspotEditor instance');
          window.hotspotEditor.handleWebXRideMessage(event);
        } else {
          console.log('hotspotEditor or handleWebXRideMessage not available');
          console.log('window.hotspotEditor:', window.hotspotEditor);
        }
      });
      
      // Check if we're running in WebXRide context
      if (window.location.search.includes('webxride=true')) {
        console.log('Initializing WebXRide integration');
        window.hotspotEditor.initWebXRideIntegration();
      } else {
        console.log('Not in WebXRide context, skipping integration');
      }
      
      // Send EDITOR_READY message to parent (bridge) if we're in a WebXRide context
      if (window.parent && window.parent !== window) {
        console.log('Sending EDITOR_READY message to parent');
        window.parent.postMessage({
          type: 'EDITOR_READY',
          data: {
            status: 'ready',
      ...
      // A-Frame not ready yet, wait a bit more
      console.log('A-Frame not ready yet, waiting...');
      setTimeout(waitForAFrame, 100);
    }
  };
  
  // Start waiting for A-Frame
  waitForAFrame();
});
