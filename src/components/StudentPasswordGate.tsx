...
export const StudentPasswordGate: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (input === STUDENT_PASSWORD) {
      localStorage.setItem('studentUnlocked', 'true');
      setUnlocked(true);
    } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-screen bg-gray-900 text-white">
        <form onSubmit={handleSubmit} className="bg-gray-800 p-8 rounded shadow-lg flex flex-col items-center">
          <label className="mb-2 text-lg font-semibold">Enter student password:</label>
          <input
            type="password"
            value={input}
            ...
            autoFocus
          />
          <button type="submit" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded text-white font-semibold">Enter</button>
