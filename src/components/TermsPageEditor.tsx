...
import { getTermsPage, updateTermsPage, AboutPage as TermsPageType } from '../lib/supabase';
import { ArrowLeft, Save, Loader2, Eye, EyeOff, Code, Palette } from 'lucide-react';
import Editor from './Editor';
import { FileType } from '../types';

interface TermsPageEditorProps {
  onBack: () => void;
  onSave?: () => void;
  currentUser: string;
}
...
export const TermsPageEditor: React.FC<TermsPageEditorProps> = ({
  onBack,
  onSave,
  currentUser,
}) => {
  ...
  const loadTermsPage = async () => {
    try {
      ...
      if (page) {
        ...
        setHtmlContent(`<h1>Terms of Use</h1>
<p>These Terms of Use govern your access to and use of the WebXRide platform.</p>`);
        setCssContent(`body {
...
p, li {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await updateTermsPage(
        title.trim(),
        htmlContent.trim(),
        cssContent.trim(),
        currentUser
      );
      console.log('Saved terms page:', result);

      if (onSave) {
        onSave();
      }
    } catch (err) {
      console.error('Error saving terms of use page:', err);
      setError(
        `Failed to save terms of use page: ${
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">
                This is how your Terms of Use page will appear.
              </span>
            </div>
